...
$html = file_get_html('http://www.ecomotori.net/distributori/metano/italy');
...
for($j=0;$j<count($es2);$j++){
    ...
    $stringaProvincia=substr($linkProvincia[0],9,57);
    $nomeProvincia[$j]=$linkProvincia[0]->plaintext;
    $l="http://www.ecomotori.net".$stringaProvincia;
    
    echo $nome."->".$nomeProvincia[$j]."<br>";
    
  /*  $str="insert into province_metano (regione,provincia,link) values ('".addslashes($nome)."','".addslashes($nomeProvincia[$j])."','".addslashes($l)."')";
...
$str="select provincia from province_metano";
...
for($k=0;$k<count($nomeProvincia);$k++){
    ...
    if($primaProvincia!=""){ //se la mia provincia esiste
        ...
        $str="select link from province_metano where provincia='".$primaProvincia."'";
        ...
        $es3=$div3->find('table td[width=180]');
           ...
           for($y=0;$y<count($es3);$y++){
               ...
               //mi faccio i controlli sulla formattazione della latitudine
                ...
                if(substr($sezione,202,1)=="e")
                        $stringaLatitudine=substr($sezione,205,7);
                
                else if(substr($sezione,202,1)==".")
                $stringaLatitudine=substr($sezione,200,7);
                
                else
                $stringaLatitudine=substr($sezione,202,7);
                
                //controlli sulle latitudini e longitudini
                if(substr($stringaLatitudine,0,1)=="(")
                        $stringaLatitudine=substr($stringaLatitudine,2);
                ...
                if(substr($sezione,214,1)==".")
                        $stringaLongitudine=substr($sezione,212,7);
                        
                else if(substr($sezione,213,1)==".")
                        $stringaLongitudine=substr($sezione,211,7);
                        
                else if(substr($sezione,215,1)=="'")
                        $stringaLongitudine=substr($sezione,216,7);
                
                else
                        $stringaLongitudine=substr($sezione,213,7);
                ...
                $str="insert into coordinate_metano (province,indirizzo,prezzo,latitudine,longitudine) values ('".addslashes($primaProvincia)."','".addslashes($indirizzo)."','".addslashes($prezzoTagliato)."','".addslashes($stringaLatitudine)."','".addslashes($stringaLongitudine)."')";
                $ris=mysqli_query($db,$str);
